...
import { Plus, CheckCircle2, Clock, AlertTriangle, ListTodo, ChevronDown, User, Calendar, ArrowUpRight } from 'lucide-react';
...
const STAT_CARDS = [
  { key: 'total',     label: 'Total Tasks',  Icon: ListTodo,      accent: 'var(--border-strong)',       iconBg: 'var(--bg-overlay)', iconColor: 'var(--text-secondary)' },
  ...
  { key: 'done',      label: 'Completed',    Icon: CheckCircle2,  accent: 'rgba(16,185,129,0.6)',       iconBg: 'rgba(16,185,129,0.08)', iconColor: '#059669' },
];
...
export default function TasksPage() {
  ...
  return (
    <div style={{ padding: '2rem 2.5rem 3rem', minHeight: '100%', boxSizing: 'border-box' }}>
            ...
            {showEmpFilter && (
              <div style={{
                ...
                {[{ id: 'all', name: 'All Members' }, ...employees.map(e => ({ id: e.id, name: getEmpName(e) }))].map(e => (
                  <button
                    key={e.id}
                    onClick={() => { setFilterEmployee(e.id); setShowEmpFilter(false); }}
                    style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
        {STAT_CARDS.map(({ key, label, Icon, accent, iconBg, iconColor }) => (
          ...
          >
            <div style={{ width: 38, height: 38, borderRadius: '9px', background: iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Icon size={17} color={iconColor} />
            </div>
            <div>
              <div style={{ fontSize: '1.625rem', fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1, letterSpacing: '-0.03em' }}>{stats[key]}</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.2rem', fontWeight: 500 }}>{label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{
          ...
          <div style={{ width: 52, height: 52, borderRadius: '14px', background: 'var(--bg-raised)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem' }}>
            <ListTodo size={24} color="var(--text-tertiary)" />
          </div>
          <p style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--text-primary)', margin: '0 0 0.375rem' }}>No tasks yet</p>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', margin: '0 0 1.5rem', textAlign: 'center', maxWidth: 280 }}>
            {filterStatus !== 'all' || filterEmployee !== 'all'
              ? 'No tasks match the current filters.'
              : 'Create your first task and assign it to a team member.'}
          </p>
          {filterStatus === 'all' && filterEmployee === 'all' && (
            <button onClick={openCreate} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'var(--accent)', color: 'var(--surface)', border: 'none', borderRadius: '8px', padding: '0.6rem 1.25rem', fontWeight: 700, fontSize: '0.875rem', cursor: 'pointer', fontFamily: 'inherit' }}>
          ...
          {filtered.map(task => {
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '0.625rem', borderTop: '1px solid var(--border-subtle)', marginTop: 'auto' }}>
                  ...
                  {dl ? (
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>No deadline</span>
                  )}
                </div>
              </div>
            );
