...
import {
  ...
  TrendingUp, ArrowRight, Trash2, AlertTriangle, Pencil,
} from 'lucide-react';
...
export default function OfferTracker() {
  ...
  // HR documents live in `records`, where the offer type is stored as 'offer'
  // (documentStore normalizes 'offer_letter' on the way in). Accept both keys
  // so rows written under either name still show up.
  //
  // Offers generated by the Employees page are excluded: that flow onboards the
  // person directly and only files the letter for download, so it has no
  // acceptance to track here.
  const offers = useMemo(
    () => allDocs.filter((d) => isOffer(d) && d.source !== 'employee_form'),
    [allDocs]
  );
  ...
  const activeList = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      list = list.filter((d) =>
        ...
        docNumber(d).toLowerCase().includes(t)
      );
    }
  ...
  // A portal link is now a server-issued credential rather than a string this
  // component can assemble. Cache one per document so Copy, Email and WhatsApp
  // on the same row share a token instead of minting three.
  ...
  const handleSendMail = async (doc) => {
    ...
    try {
      ...
      // The tracker reports acceptance status "for all sent offer letters", so
      // sending is what moves an offer out of draft/pending. /api/portal then
      // takes it to 'viewed' when the candidate opens the link.
  ...
  // Editing is only honest while the offer is still open: once the candidate
  // has signed or declined, the letter they responded to is the record.
  const RESPONDED = new Set(['signed', 'accepted', 'declined', 'acknowledged',
    'fully_signed', 'cancelled']);
  const canEdit = (doc) => !RESPONDED.has(doc.status);

  const handleEdit = (doc) => {
    // The recipient's email is baked into the portal token, so a fresh link is
    // minted on the next Copy/Email rather than reusing the cached one.
    linkCache.current.delete(doc.id);
    setEditOffer(doc);
  };
  ...
  const handleDelete = async (doc) => {
    ...
    try {
      ...
      alert('Could not delete the document: ' + err.message);
    }
    // Realtime DELETE events carry no org_id to match the channel filter, so
    // repaint from the store's cache rather than waiting for one.
    applyDocs(orgStore.getSection('records'));
  };
    ...
    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
      ...
      {isOffer(doc) && (
        <ActionBtn
          ...
          title={canEdit(doc) ? 'Edit offer details' : 'This offer has already been responded to'}
          fullWidth={fullWidth}
        >
          <Pencil size={14} />
          {fullWidth && <span>Edit</span>}
        </ActionBtn>
      )}
    ...
    <div>
      ...
      {editOffer && (
        <CreateOfferModal
          activeOrg={activeOrg}
          offer={editOffer}
          onClose={() => { setEditOffer(null); applyDocs(orgStore.getSection('records')); }}
        />
      )}
      ...
      {showCreate && (
        <CreateOfferModal
          activeOrg={activeOrg}
          onClose={() => { setShowCreate(false); applyDocs(orgStore.getSection('records')); }}
        />
      )}
...
// A saved offer speaks the document vocabulary; the form speaks the form's.
function formFromOffer(offer) {
  if (!offer) return EMPTY_OFFER_FORM;
  return {
...
// `offer` set ⇒ edit an existing letter in place; absent ⇒ create a new one.
function CreateOfferModal({ activeOrg, onClose, offer }) {
  const winW = useWindowWidth();
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Keep the offer's own department in the list even if no employee or
      // department record carries it — otherwise editing silently clears it.
      const existing = offer?.department ? [offer.department] : [];
      setDeptOptions([...new Set([...fromEmps, ...fromFb, ...existing])].sort());
    });
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        // Keep everything the document already carries — status, doc number,
        // company snapshot, signature fields — and overwrite only what the
        // form owns. The portal link is bound to the document id, so it keeps
        // working and simply serves the corrected letter.
      ...
      alert(isEdit ? 'Failed to save changes: ' + err.message : 'Failed to create offer: ' + err.message);
    } finally {
      setSaving(false);
    }
  };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem 1.25rem', borderBottom: '1px solid var(--border-default)', flexShrink: 0 }}>
          <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)' }}>
            {created ? 'Offer Created' : isEdit ? 'Edit Offer Letter' : 'New Offer Letter'}
          </h3>
        ...
        {created ? (
          ...
          <form onSubmit={handleCreate} style={{ overflowY: 'auto', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <button type="submit" className="easy-submit" disabled={saving} style={{ margin: 0 }}>
              {saving
                ? <><Loader size={16} className="spin-icon" /> {isEdit ? 'Saving...' : 'Creating...'}</>
                : isEdit
